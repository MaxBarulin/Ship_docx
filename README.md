...
| Маршрут | кольцо Самара — Казань — Уфа — Самара по Волге, Каме и Белой, 15 дней, 2606 км; класс РРР «О 2,0» |
...
- **Узел для КД сменён 22.09.2026.** Раньше была ось плицы гребного колеса
  с кривошипом (в плане команды §7 ещё она). Теперь узел — литой
  фундамент-замок для крепления модулей солнечной палубы: он же по заданию
  Глеба. Ось плицы осталась проработкой колеса (`docs/проект/узел_ось_плицы.md`),
  в КД не идёт.
...
| **Маша** | интеграция, ПЗ, презентация, персонал, риски | `docs/проект/записка.md` целиком — она сводит; разделы 5 и 8; `docs/проект/персонал_и_дорожная_карта.md`; схемы `02_дорожная_карта.png`, `03_оргструктура.png`; `docs/команда/README.md` — что из ваших файлов встроено | записка 5.3…5.5 (подбор, оплата, обучение — 2,5); подтвердить оценки и мероприятия реестра рисков (3,5 частично); кооперация по судну (1,5); презентация (3,5); все разделы и вёрстка ПЗ в А4 (2); сквозная проверка «вход → выход» (2) |
...
| **Алексей** | КД узла, работоспособность идеи | СБ и детали: `CAD/узел/ВГ-2026_46_*.dxf` (растр — `renders/горизонт_2026/чертежи_dxf/ВГ-2026_46_*.png`); 3D — `CAD/STEP/VG-2026_46_*.step`; спецификация; рендеры `узел/46_01…03`; эксплуатация — записка раздел 9 | DWG листов узла и МР-01 пересобрать на машине с AutoCAD: `python scripts/чертежи_dwg.py`; просмотреть КД глазами конструктора |
