...
CAD/src/twistlock.py (корпус - модель конструктора из STEP). Каждая деталь - отдельный
объект (связанные копии одной сетки), швы glTF сшиваются, чтобы аудит замкнутости видел
тела, а не лоскуты. Начало узла - плечо корпуса под фитинг, настил на 77 мм ниже.
Замок под модулем - «закрыто», на свободном слоте - «открыто»: рукоятка во втором или
первом вырезе корпуса, поворот запора T.ПОВОРОТ. Прорезь с рукояткой смотрит в торцевой
проход: у кормовых опор слота - в корму, у носовых - в нос.
...
Z_УЗЛА = Z_ПЛОЩАДКА                                                             # начало узла - плечо корпуса
К = M.КОНТЕЙНЕР
...
#: Шайба поз. 5 и втулки скрыты внутри - не ставятся.
ДЕТАЛИ = {"pos.1 ": "Краска_ПФ115", "pos.2 ": "Краска_ПФ115", "pos.3 ": "Краска_ПФ115", "pos.4 ": "Краска_ПФ115",
          "pos.6 ": "СТЭФ", "pos.9 ": "АМг5_лист"}
ПОВОРОТНЫЕ = ("pos.2 ", "pos.3 ")                    # поворачиваются рукояткой на T.ПОВОРОТ при запирании
...
def материалы():
    ...
    _материал("Краска_ПФ115", (0.16, 0.012, 0.02, 1.0), 0.15, 0.45)     # эмаль ПФ-115 бордовая по технологии
...
def _угол_опоры(sx):
    """Поворот узла. Прорезь корпуса с рукояткой (-Y узла) - в торцевой проход: у кормовых фитингов слота в корму
    (-90°), у носовых в нос (+90°). Длинная сторона центратора (Y узла) ложится вдоль контейнера."""
    return -0.5 * math.pi if sx < 0 else 0.5 * math.pi
...
def фундаменты(кол, занятые, открытые=()):
    ...
    for f in M.фундаменты():
        ...
        for ключ, материал in ДЕТАЛИ.items():
            me, mw = п[ключ]
            ob = bpy.data.objects.new("%s_%s" % (имя, ключ.strip().replace(" ", "_")[:22]), me)
            кол.objects.link(ob)
            доп = math.radians(T.ПОВОРОТ) if (закрыто and ключ in ПОВОРОТНЫЕ) else 0.0
            ob.location = (f["x"], f["y"], Z_УЗЛА)
            ob.rotation_euler = (0.0, 0.0, база + доп)
            _назначить(ob, материал)
            n += 1
        # головки болтов на платике: координаты болтов узла, повёрнутые вместе с узлом
        bx, by = T.ПЛАТИК["болт_x"] / 1000.0, T.ПЛАТИК["болт_y"] / 1000.0
        c, s_ = math.cos(база), math.sin(база)
        z_h = Z_УЗЛА + (T.КОРПУС["z"][0] + 7.5) / 1000.0                      # головка 15 мм на верхе платика
        for k, (dx0, dy0) in enumerate([(-bx, -by), (-bx, by), (bx, -by), (bx, by)]):
            dx, dy = c * dx0 - s_ * dy0, s_ * dx0 + c * dy0
