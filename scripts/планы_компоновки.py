    ...
    if палуба == "второе дно":
        ...
        for t in G.TANKS:
            ...
            if "y0" in t:
                ...
                # в две строки: у коротких цистерн однострочные подписи наезжали на соседние
                ax.text(xc, (t["y0"] + t["y1"]) / 2.0, "%s\n%.0f м³" % (t["code"], t["vol"]),
                        ha="center", va="center", fontsize=6.0, color=INK, zorder=5, linespacing=1.1)
                if t["y0"] < 0:
                    continue
                ax.text(xc, 9.85, t["name"].replace(" ПБ", "").replace(" ЛБ", "") + " (ЛБ/ПБ)",
...
def build(verbose=True):
    и = GA.итоги()
    листы = [
        ("второе дно", "0. Второе дно, 0,00…0,90 м", "цистерны второго дна, продольные переборки ±%.2f м" % G.LONG_BULKHEAD_Y),
        ...
        ("солнечная", "4. Солнечная палуба, 8,60 м",
         ...
         "темы модулей - на схеме модульного решения" % (len(MOD.слоты()), len(MOD.фундаменты()))),
    ]
    пути = []
