...
Листы ОР-01…ОР-07: планы главной, средней и солнечной палуб, трюм и второе
дно с цистернами и механизмами, продольный разрез, поперечные сечения,
теоретический чертёж. Единицы — миллиметры, судно вычерчено в натуральную
...
открывается AutoCAD и nanoCAD как родной формат и сохраняется в .dwg
...
def deck_plan(палуба, mark, title, extra=None):
    ...
    if палуба == "солнечная":
        ...
        text(msp, 0.5 * (р["x0"] + р["x1"]) * K, 0.0, "рулевая рубка (подъёмно-опускная)", 2.4, sc, "02_ПАЛУБЫ")
        X = W.X_AXIS
...
def profile():
    ...
    notes = ("Продольный разрез по ДП. Высоты от основной плоскости, м.",
             "Габаритная высота от ВЛ %.2f м с опущенной рубкой, %.2f м с поднятой." % (G.AIR_DRAFT, G.AIR_DRAFT_RAISED),
