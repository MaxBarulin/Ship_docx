...
fig.text(0.012, 0.938,
         ...
         "шпаций 550 мм, магистрали и шахты не трогаются.",
...
ax.set_xlim(0, R.CLASSIC_AREA * 1.10)
...
for i, s in enumerate(rows):
    ...
    for k in ("эконом", "стандарт", "бизнес", "люкс"):
        ...
        if w >= 215:
            ...
            dy = 0.40 + 0.18 * (narrow % 2)
            ax.plot([x + w / 2, x + w / 2], [i - 0.26, i - dy + 0.04],
                    color=COL[k], lw=0.9)
            ax.text(x + w / 2, i - dy, lab, ha="center", va="bottom",
                    color=COL[k], fontsize=8.5)
            narrow += 1
        x += w
    ax.text(0, i - 0.68, "%s  «%s»" % (s["code"], s["name"]),
            fontsize=12, fontweight="bold", color=INK)
    ax.text(x + 24, i, "%.0f м²" % s["cabin_area"], va="center", fontsize=10, color=GRY)
ax.text(0, len(rows) - 0.55, "Площадь пассажирских кают, м² — у всех версий одна и та же",
...
for i, s in enumerate(rows):
    ...
    ax3.text(0.004, y - 0.062, s["idea"] + ". " + s["note"], fontsize=9, color=GRY)
