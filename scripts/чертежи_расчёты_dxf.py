...
def rt01():
    ...
    for z, имя in доп:
        ...
        ЧД.text(msp, 9.1 * K, z * K, "ВЛ " + имя, 2.4, sc, "07_ОСИ", TA.MIDDLE_LEFT)
    ...
    notes = ("Обводы: плоское днище, скуловая дуга, касательная к днищу и к борту, прямой борт с развалом — gorizont_lines.",
...
def rt02():
    ...
    doc, msp = лист_графиков("ВГ-2026 РТ-02 кривые элементов", "Кривые элементов теоретического чертежа",
                             ...
                             ("Гидростатика — gorizont_hydro.hydrostatics(T) по обводам с нишами колёс, шаг по длине 0,5 м.",
...
def rt03():
    ...
    doc, msp = лист_графиков("ВГ-2026 РТ-03 строевая и нагрузка", "Строевая по шпангоутам и нагрузка масс",
                             ...
                             ("Слева корма. Площади погружённых сечений omega(x) при осадке %s м — gorizont_hydro.section_area." % ф(T, 2),
                              "Нагрузка масс w(x) — по группам gorizont_hydro.groups(): корпус по площади обшивки, остальное равномерно по границам групп.",
...
def rt04():
    ...
    doc, msp = лист_графиков("ВГ-2026 РТ-04 остойчивость", "Остойчивость на больших углах крена",
                             ...
                             ("Сплошная — корпус до высоты борта %s м, штриховая — с закрытым ярусом главной палубы до %s м." % (ф(G.DEPTH, 2), ф(zt, 2)),
                              "Пантокарены — gorizont_hydro.pantocarene по обводам с нишами; ДСО GZ = l_k − zg·sin(theta), ДДО — интеграл ДСО.",
...
def rt06():
    ...
    doc, msp = лист_графиков("ВГ-2026 РТ-06 ходкость", "Ходкость: сопротивление, мощность, мелководье",
                             ...
                             ("Сопротивление на глубокой воде: трение с формфактором + остаточное — gorizont_hydro.resistance.",
                              ...
                              "Мелководье: критическая скорость sqrt(g·H), предел 0,7 от неё, ограничение по мощности — gorizont_hydro; просадка по Баррасу — gorizont_manoeuvre."))
...
def rt07():
    ...
    doc, msp = лист_графиков("ВГ-2026 РТ-07 электробаланс", "Электробаланс по режимам и выбор мощности ГДГ",
                             ...
                             ("Потребность на шинах ГРЩ по режимам — gorizont_power.table(): судовые и бытовые потребители плюс гребные ГЭД.",
    ...
    # таблица режимов
    таб = [[r["name"], ф(r["hotel"], 0), ф(r["propulsion"], 0), ф(r["total"], 0), "%d" % r["dg"], ф(100 * r["load"], 0) + " %", "да" if r["ok"] else "нет"] for r in rows]
...
def rt08():
    ...
    r = St.stresses()
    bbox = (-2.0 * K, -1.8 * K, 20.5 * K, 5.2 * K)     # справа от сечения и выносок — таблицы
    sheet, sc = ЧД.pick_sheet(bbox[2] - bbox[0], bbox[3] - bbox[1])
    doc = newdoc(); msp = doc.modelspace()
    ...
    ЧД.text(msp, XT, yb - 3.5 * sc, "Результаты расчёта общей продольной прочности", 3.2, sc, "08_ТЕКСТ", TA.BOTTOM_LEFT)
    ЧД.table(msp, XT, yb - 4.0 * sc, sc, ["Величина", "Значение"], итоги, [56, 60], h_row=5.0)
    notes = ("Мидель-шпангоут по шп. %d (x = %s м), правый борт; левый симметричен. Толщины — gorizont_struct.PLATES, профили — PROFILES." % (round(X / S.SPACING), ф(X, 0)),
...
def rt09():
    ...
    notes = ("Набор — gorizont_struct: шпация %d мм, рамная %d мм; переборки — gorizont_hydro.BULKHEADS (общие с ОР-04 и расчётом непотопляемости)." % (round(SP * 1000), round(FS * 1000)),
