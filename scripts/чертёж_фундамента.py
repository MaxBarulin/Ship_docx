...
def корпус_лист():
    ...
    L.pl([(-150, 185), (-150, 160)]); L.pl([(-153, 166), (-150, 160), (-147, 166)]); L.text(-158, 182, "В", 5.0)
    # --- разрез Б-Б
    L.вид(565.0, 372.0)
...
def отливка_лист():
    ...
    L.note(-Dн / 2 - 10, H - 20, -160, 250, "нога-знак окна", 2.5)
    L.note(-40, -п_н - 25, -160, -35, "нижний знак Ø%s × %s" % (ф(Dв), ф(T.ЗНАК_НИЗ)), 2.5)
    ...
    L.note(-Lx / 2 - 20, -п_н + pit_h, -160, 40, "питатель 40/32 × %s" % ф(pit_h), 2.5)
...
def плита_лист():
    ...
    notes = ("Размеры моделей даны с учётом линейной усадки стали 20ГЛ %s %%; формовочные уклоны 1° по ГОСТ 3212-92." % ф(T.УСАДКА, 1),
             "Модели — полиуретановый модельный пластик плотностью 0,7…1,2 г/см³, фрезеровать по 3D отливки CAD/STEP/VG-2026_46_01_casting.step.",
