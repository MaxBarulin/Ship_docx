...
"""
import glob, json, os, re, runpy, subprocess, sys, tempfile
...
def _печать(имя, р):
    плохо = len(р["имена"]) + len(р["код"]) + len(р["наложения"]) + len(р["за_краем"])
    print("%s %-62s имена %d  код %d  наложений %d  за краем %d  поручения %d" % (
        "!!" if плохо else "  ", имя[-62:], len(р["имена"]), len(р["код"]), len(р["наложения"]), len(р["за_краем"]),
        len(р["поручения"])))
    for k in ("имена", "код", "наложения", "за_краем", "поручения"):
        for s in р[k][:6]:
            print("     %-10s %s" % (k, s))
    return плохо
...
if __name__ == "__main__":
    ...
    n = проверить_dxf() if режим == "dxf" else проверить_картинки(sys.argv[2:] or None)
    print("замечаний: %d" % n)
