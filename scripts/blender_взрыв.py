...
СБОРКИ = [
    ...
    (("Палуба_солнечная", "Ограждение_солнечной", "Солнечные_модули", "Труба", "Кнехт", "Якорная"), (0.0, 0.0, 19.0)),
...
def подписи():
    ...
    пометки = [("Корпус, второе дно, цистерны, трюм: ГДГ, приводы колёс", 0.0),
               ...
               ("Солнечная палуба: навес с модулями, труба", 19.0),
               ("Рубка подъёмно-опускная", 24.0)]
    сделано = []
    for i, (т, dz) in enumerate(пометки):
        ...
        # столбик подписей перед судном, снизу вверх в порядке сборок
        ob.location = (6.0, -44.0, 1.0 + dz * 0.62)
