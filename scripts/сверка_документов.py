...
DOCS = [
    ...
    os.path.join(ROOT, "docs", "проект", "концепция.md"),
    os.path.join(ROOT, "docs", "проект", "записка.md"),
]
...
def facts():
    ...
    out = [
        ...
        ("класс РКО", G.RRR_CLASS,
         ["Класс РКО «О»", "нормы РКО класса «О»",
          "Лед 1 / Arc 4 | принят"]),
        ("осадка", "%.2f м" % e["T"], ["осадке 2.22 м", "**2.22 м**", "осадка 1,29", "осадка 1.29"]),
        ("водоизмещение", "%.0f т" % e["D"], ["весит 3990 т", "3990 т (", "2458 т", "2461 т"]),
        ("габаритная высота", "%.1f м" % G.AIR_DRAFT, []),
        ("пассажиров", str(GA.итоги()["пассажиров"]), ["212 мест", "220 мест", "212 чел"]),
        ("экипаж", str(G.CREW), ["57 чел", "экипаж 57"]),
        ("критерий погоды", "%.1f" % w["K"], ["Критерий погоды 14.9"]),
        ("метацентрическая высота", "%.2f м" % w["h"],
         ["h = 7.84 м", "h = 7.69 м"]),
        ("период бортовой качки", "%.2f с" % H.roll_period(), []),
        ("пассажирских кают", str(GA.итоги()["пассажирских_кают"]), ["106 кают"]),
    ]
    out.append(("водонепроницаемых переборок", str(len(GA.ПЕРЕБОРКИ)), []))
