...
#
# Настил главной палубы 8 мм, а не 10: после переноса танков СПГ к миделю и
# снятия лишнего веса изгибающий момент на перегибе упал, и десятка стала
# избыточной. С восьмёркой перегиб выбирает 167 МПа из 195 — 86 %, запас
# четырнадцать процентов. Семёрка дала бы 92 %, и это слишком тонко для
# стадии, на которой обводы ещё могут поехать.
...
X_MID = 80.0   # расчётное сечение вне ниши колеса (62,4…72,1): полная ширина
...
def superstructure_weight():
    ...
    add("Рубка стационарная: крыша и стены", 9.0 * 8.0 + 2 * (11.0 + 9.2) * 2.5,
