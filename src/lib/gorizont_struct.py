...
DB_HEIGHT = 0.90          # высота двойного дна до настила первой палубы, м
...
# Настил главной палубы, палубный стрингер и ширстрек утолщены против
# первоначальных 8/10/10 мм: при классе «М» (высота волны 3,0 м) расчётный
# изгибающий момент на прогибе даёт в палубе 205 МПа при допускаемых 195,
# а с 10/12/11 мм — 180 МПа. Палуба здесь слабее днища: нейтральная ось
# лежит на 1,81 м от основной, и до палубы от неё дальше, чем до днища.
PLATES = {
    "горизонтальный киль":      11.0,
    "днище":                     7.5,
    "скула":                     8.5,
    "борт в районе ВЛ":          7.5,
    "борт ниже пояса":           7.0,
    "ширстрек":                 10.0,
    "второе дно":                7.0,
    "настил главной палубы":    10.0,
    "палубный стрингер":        12.0,
    "переборки водонепроницаемые": 6.0,
    "настил средней палубы":     6.0,
    "настил солнечной палубы":   6.0,
...
def steel_weight(step=1.0):
    ...
    # толщины берутся из PLATES, а не вписываются числом: иначе изменение
    # толщины меняет напряжение, но не массу, и нагрузка масс врёт
    t_bot = (PLATES["днище"] * 0.9 + PLATES["горизонтальный киль"] * 0.1)
    t_sid = (PLATES["борт ниже пояса"] * 0.55 + PLATES["борт в районе ВЛ"] * 0.25
             + PLATES["ширстрек"] * 0.20)
    add("Обшивка днища с горизонтальным килем", bot, t_bot, zb / max(bot, 1e-9))
    add("Скуловой пояс", bil, PLATES["скула"], 0.35)
    add("Обшивка борта", sid, t_sid, zs_ / max(sid, 1e-9))
    ...
    a_deck = deck_area(2, G.LOA - 2, G.DEPTH)
    a_str = 2 * 1.20 * (G.LOA - 8.0)            # палубный стрингер, полоса 1,2 м
    add("Настил главной палубы", a_deck - a_str, PLATES["настил главной палубы"],
        G.DEPTH)
    add("Палубный стрингер", a_str, PLATES["палубный стрингер"], G.DEPTH)
    add("Второе дно (настил первой палубы)",
        deck_area(2, G.LOA - 10, DB_HEIGHT, 0.92), PLATES["второе дно"], DB_HEIGHT)
    ...
    add("Водонепроницаемые переборки %d шт" % (nb - 1), ab,
        PLATES["переборки водонепроницаемые"], G.DEPTH / 2)

    # --- набор корпуса
    Lm = G.LOA * 0.78                # длина, на которой идёт продольный набор
    n_bot = 22
    n_db = 24
    n_dk = 24
    for nm, n, prof, z in (("Рёбра жёсткости днища", n_bot, "ребро днища", 0.10),
                           ("Рёбра жёсткости второго дна", n_db,
                            "ребро второго дна", DB_HEIGHT),
                           ("Рёбра жёсткости главной палубы", n_dk,
                            "ребро палубы", G.DEPTH - 0.10)):
    ...
    for nm, prof, n, z in (("Вертикальный киль", "вертикальный киль", 1, 0.45),
                           ...
                           ("Карлингсы главной палубы", "карлингс", 3, G.DEPTH - 0.35)):
        hw, tw, bf, tf = PROFILES[prof]
        a = (hw * tw + bf * tf) * 1e-6 * n * Lm
        items.append(dict(name=nm, area=a, t=0.0,
                          m=a * RHO_STEEL, z=z, x=G.LOA / 2))
    # флоры и шпангоуты
    ...
    a_bm = profile_area("рамный бимс") * 1e-4 * (G.SUPER_BEAM - 0.5) * n_rs
    ...
    # Ниши колёс — сквозные вырезы в борту в корме. Каждый требует рамного
    # набора по контуру, комингса и усиленной палубы над вырезом: это не
    # «плюс процент на всякий случай», а конкретные связи вокруг дыры,
    # через которую корпус должен продолжать передавать изгиб.
    niches = 0.035 * m
    items.append(dict(name="Окантовка и подкрепление ниш колёс (3,5 %)",
                      area=0.0, t=0.0, m=niches, z=1.80, x=G.LOA / 2))
    m += niches
...
def superstructure_weight():
    ...
    Два яруса над главной палубой вместо прежних трёх. Крыша верхнего
    яруса — солнечная палуба, и на ней стоят танки СПГ, поэтому она взята
    той же толщины, что настил, а не тоньше.
    """
    items = []
    ...
    L = G.SUPER_END - G.SUPER_START
    B = G.SUPER_BEAM
    h = G.DECK_PITCH
    z_ср, z_сол = G.DECKS["средняя"], G.DECKS["солнечная"]
    add("Настил средней палубы", L * B, PLATES["настил средней палубы"], z_ср)
    add("Настил солнечной палубы", L * B, PLATES["настил солнечной палубы"], z_сол)
    add("Борта надстройки, ярус главной палубы", 2 * L * h,
        PLATES["борта надстройки"], G.DECKS["главная"] + h / 2)
    add("Борта надстройки, ярус средней палубы", 2 * L * h,
        PLATES["борта надстройки"], z_ср + h / 2)
    add("Поперечные переборки надстройки", 16 * B * (h - 0.2),
        PLATES["борта надстройки"], z_ср)
    add("Рубка подъёмно-опускная с шахтой", 9.0 * 8.0 + 2 * 2.6 * 11.0,
        PLATES["рубка"], G.WHEELHOUSE_FLOOR + 1.2)
    m = sum(i["m"] for i in items)
    extra = 0.22 * m          # набор надстройки, стойки, кницы
    items.append(dict(name="Набор надстройки, стойки, кницы (22 %)",
                      area=0.0, t=0.0, m=extra, z=z_ср))
    m += extra
    z = sum(i["m"] * i["z"] for i in items) / m
    return dict(items=items, m=m, z=z)
