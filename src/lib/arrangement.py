...
LAYOUT: dict[str, list[Zone]] = {
    ...
    # Солнечная палуба разложена по обводу, а не по остаточному принципу.
    # Спортивная площадка стоит в середине, где палуба во всю ширину:
    # в носу обвод сужается вдвое, и площадка туда просто не помещалась —
    # зона выпадала из расстановки целиком, а корма оставалась голой.
    # Труба получила собственную зону: раньше она стояла посреди чужой.
    "солнечная": [
        Zone("open", "Шезлонги и зона отдыха", 20_000),
        Zone("open", "Спортивная площадка", 12_000),
        Zone("open", "Бассейн и бар у бассейна", 18_000),
        Zone("open", "Солнечные панели", 10_000),
        Zone("open", "Кожух трубы и вентиляция", 8_000),
        Zone("open", "Смотровая площадка", 14_000),
    ],
}
