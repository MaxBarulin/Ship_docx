...
Методика - курс теории корабля СПбГМТУ:
  ...
  * остойчивость на больших углах - пантокарены через отсечение сечения
    ...
    остойчивости, проверка по нормам РКО;
...
def niche_half(x):
    """Стенка ниши колеса - внутри ниши обвод не шире её стенки.

    Ниша идёт от киля до палубы (gorizont_wheel.NICHE_Z0 = 0), поэтому
    ограничение действует на всех высотах. Вне ниши - без изменений.
    """
    global _WHEEL
    if _WHEEL is None:
        from . import gorizont_wheel as _W
        _WHEEL = _W
    return _WHEEL.niche_half(x)
...
def profile(x):
    ...
    В районе колёс полуширота срезана стенкой ниши - это часть обводов,
    а не вырез в готовом корпусе - гидростатика, строевая и теоретический
    чертёж считаются с нишей.
    """
    ...
    for z in zs:
        pts.append((z, min(section_y(z, zk, bk, zb, bb, phi), yn)))
    pts.append((zb, min(bb, yn)))
...
def half_breadth(x, z):
    ...
    В нише колеса - не шире стенки ниши.
    """
    zk, bk, zb, bb, phi = _column(x)
    return min(section_y(z, zk, bk, zb, bb, phi), niche_half(x))
...
def super_half_breadth(x):
    ...
    Считает `gorizont_super`. Раньше обвод жил здесь, а модель строила
    надстройку коробкой постоянной ширины, и парусность расходилась с тем,
    что видно на рендере. Теперь источник один.
    """
    from . import gorizont_super as _SU
    return max(_SU.полуширота(x, я) for я in _SU.ПОРЯДОК)
...
# Нагрузка масс: (группа, масса т, x0, x1, z ЦТ, момент свободной поверхности т·м).
# Постоянные группы - здесь; механизмы трюма и содержимое цистерн
# подставляются из gorizont.ТРЮМ и gorizont.TANKS, чтобы нагрузка масс,
# чертёж трюма и модель описывали одно размещение.
_FIXED_LIGHT = [
    ("Корпус металлический",               846.0,   0.0, 140.0,  1.45,   0.0),
    ("Надстройка и рубка",                 281.0,   5.0, 130.0,  6.60,   0.0),
    ("Изоляция и зашивка, первая",          33.0,   5.0, 130.0,  1.80,   0.0),
    ("Изоляция и зашивка, главная",         46.0,   5.0, 130.0,  4.20,   0.0),
    ("Изоляция и зашивка, средняя",         46.0,  15.0, 118.0,  7.00,   0.0),
    ("Оборудование помещений, первая",      70.0,   5.0, 130.0,  1.90,   0.0),
    ("Оборудование помещений, главная",    125.0,   5.0, 130.0,  4.30,   0.0),
    ("Оборудование помещений, средняя",    115.0,  15.0, 118.0,  7.10,   0.0),
    ("Якорно-швартовное устройство",        24.0, 118.0, 136.0,  3.60,   0.0),
    ("Спасательные средства (плоты)",       24.0,  30.0, 110.0,  8.80,   0.0),
    ("Системы и трубопроводы",              165.0,   4.0, 136.0,  3.40,   0.0),
    # Колёса с плицами, валы, подшипники ниши; ГЭД и редукторы - в ТРЮМ.
    ("Гребные колёса с валами",              62.0,  62.0,  70.0,  2.60,   0.0),
    ("Система метанола - коффердамы, насосы, газоотвод", 20.0, 40.0, 104.0, 0.60, 0.0),
    ("Установка очистки стоков - трубопроводы, автоматика", 8.0, 5.0, 14.0, 1.20, 0.0),
    ("Электрооборудование и АСУ",           112.0,   6.0, 136.0,  3.60,   0.0),
    ("Гараж, аппарель и его системы",        26.0,   5.0,  20.0,  3.40,   0.0),
    ("Запас водоизмещения 3 %",              66.0,   0.0, 140.0,  3.50,   0.0),
]
_FIXED_DEAD = [
    ("Провизия и снабжение",                 28.0,  76.0,  86.0,  1.80,   0.0),   # кладовые под камбузом
    ("Автомобили 4 x 3,5 т",                 14.0,   6.0,  19.0,  3.60,   0.0),
]
#: Момент свободной поверхности по роду груза, т·м на цистерну (частично
#: заполненную): топливо и вода в цистернах второго дна невысоких и узких.
_FS_BY_KIND = {"ЦМ": 0.0, "ЦЗ": 6.0, "ЦП": 17.0, "ЦО": 9.0, "ЦБ": 6.0, "ЦА": 12.0,
               "ЦЛ": 5.0, "ЦМС": 5.0, "ЦБЛ": 0.0}


def _tank_rows():
    """Содержимое цистерн при полном запасе, балласт в расчётном состоянии пуст."""
    rows = []
    for t in G.TANKS:
        kind = t["code"].split("-")[0]
        if kind == "ЦБЛ":
            continue
        rows.append((t["code"] + " " + t["name"] + " %.0f м3" % t["vol"],
                     round(t["vol"] * t["rho"], 1), t["x0"], t["x1"],
                     round(0.5 * (t["z0"] + t["z1"]), 2), _FS_BY_KIND.get(kind, 5.0)))
    return rows


def _hold_rows():
    return [(code + " " + name, m, x0, x1, round(0.5 * (z0 + z1), 2), 0.0)
...
def _people_rows():
    ...
    и = _GA.итоги()
    return [("Экипаж %d чел. с багажом" % G.CREW, round(0.12 * G.CREW, 1), 26.0, 110.0, 4.00, 0.0),
            ("Пассажиры %d чел. с багажом" % и["пассажиров"], round(0.15 * и["пассажиров"], 1),
             15.0, 130.0, 5.00, 0.0)]


WEIGHT_GROUPS = _FIXED_LIGHT + _hold_rows() + _FIXED_DEAD + _tank_rows() + _people_rows()
LIGHTSHIP_GROUPS = len(_FIXED_LIGHT) + len(_hold_rows())
...
def groups():
    ...
    if _GROUPS is None:
        ...
        k_к = G.HULL_MASS_FACTOR if G.HULL_HTS else 1.0
        g[0] = (g[0][0] + (" (сталь D36)" if G.HULL_HTS else ""), round(hull["m"] * k_к, 1),
                g[0][2], g[0][3], round(hull["z"], 2), g[0][5])
        k_над = G.SUPERSTRUCTURE_MASS_FACTOR[G.SUPERSTRUCTURE_MATERIAL]
        g[1] = ("Надстройка и рубка (%s)" % G.SUPERSTRUCTURE_MATERIAL,
                round(sup["m"] * k_над, 1), g[1][2], g[1][3],
                round(sup["z"], 2), g[1][5])
        if G.OUTFIT_LIGHT:
            for i, row in enumerate(g):
                if row[0].startswith("Оборудование помещений"):
                    g[i] = (row[0] + ", лёгкая отделка", round(row[1] * G.OUTFIT_MASS_FACTOR, 1),
...
def windage(T):
    ...
    from . import gorizont_super as _SU
    d = G.DECKS
    я1, я2 = _SU.ЯРУСЫ["главная"], _SU.ЯРУСЫ["средняя"]
    L1 = я1["x1"] - я1["x0"]
    L2 = я2["x1"] - я2["x0"]
    parts = [
        (G.LOA * 0.96, G.DEPTH - T, T),                       # надводный борт
        (L1, d["средняя"] - d["главная"], d["главная"]),      # ярус главной (кожухи внутри)
...
def weather_criterion(cls=None, z_top=None):
    """Критерий погоды K = M_опр / M_кр по нормам РКО."""
    cls = cls or CLASS
...
def rrr_checks(cls=None, z_top=None):
    ...
    # Угол максимума ДСО. Правила РКО требуют 25 град, но для широких судов
...
# Водонепроницаемые переборки - те же, что в общем расположении
# (gorizont_ga.ПЕРЕБОРКИ): расчёт непотопляемости и чертёж трюма читают
# один список. Отсеков ровно на один больше, чем переборок.
def _bulkheads():
    from . import gorizont_ga as _GA
    return [0.0] + list(_GA.ПЕРЕБОРКИ) + [G.LOA]


BULKHEADS = _bulkheads()
COMPARTMENTS = [
    "Ахтерпик, рулевое",                       # 0…5
    "Очистка стоков, цистерны стоков",         # 5…14
    "Вспомогательные механизмы, запальное топливо",  # 14…26
    "Насосное, пресная вода, балласт",         # 26…40
    "Аварийный сточный, балласт дифферентовки",  # 40…54
    "Метанол, кормовая группа (коффердамы)",   # 54…62
    "Машинное отделение - ГДГ, приводы колёс",  # 62…76
    "ГРЩ, преобразователи, кладовые, вода",    # 76…90
    "Метанол, носовая группа, ЦКВ",            # 90…104
    "Аккумуляторы, балласт носовой",           # 104…118
    "Водомёт, цепной ящик",                    # 118…130
    "Форпик",                                  # 130…135
    "Таранный отсек",                          # 135…140
]
