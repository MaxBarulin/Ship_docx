...
WEIGHT_GROUPS = [
    ...
    # Жидкость успокоительных цистерн: масса, аппликата и момент свободной
    # поверхности берутся из gorizont_roll, чтобы настройка цистерны и
    # нагрузка масс не разъезжались.
    ("Успокоительные цистерны, жидкость",    83.5,  46.0,  96.0,  0.23,
     70.0),
    ...
    ("Рулевое устройство",                  16.0,   2.0,   6.0,  2.10,   0.0),
    ("Спасательные средства",               62.0,  39.0,  99.0, 10.90,   0.0),
    ("Системы и трубопроводы",             260.0,   5.0, 132.0,  5.20,   0.0),
    ("ГДГ, ГРЩ и вспомогательные механизмы", 268.0, 12.0,  34.0,  1.60,   0.0),
    ("ГЭД, валопроводы, винты и насадки",    58.0,   3.0,  18.0,  1.20,   0.0),
    ("Подруливающие устройства",             16.0,  10.0, 102.0,  0.95,   0.0),
    ("Котельная установка и утилизация",     20.0,  14.0,  24.0,  2.30,   0.0),
    ("Установка очистки стоков AWTS",        22.0,  70.0,  92.0,  1.60,   0.0),
    ("Электрооборудование и АСУ",           180.0,   8.0, 132.0,  5.60,   0.0),
    ("Запас водоизмещения 3 %",              94.0,   0.0, 139.0,  5.00,   0.0),
    ("Топливо 295 м3",                      253.7,  18.0,  70.0,  0.78, 130.0),
    ("Масло и шлам 21 м3",                   19.2,  22.0,  32.0,  1.35,   8.0),
    ("Пресная вода 190 м3",                 190.0,  58.0,  80.0,  0.72,  75.0),
    ("Техническая и очищенные воды 60 м3",   60.0,  72.0,  81.0,  0.70,  36.0),
    ("Буферные и аварийная сточные 55 м3",   55.0,  70.0,  92.0,  0.70,  45.0),
    ("Льяльные воды 10 м3",                  10.0,  30.0,  34.0,  0.28,  12.0),
...
LIGHTSHIP_GROUPS = 21
# Балласт 400 м3 в таблицу не входит: в расчётном состоянии «полный запас»
...
COMPARTMENTS = [
    "Ахтерпик и румпельное", "Помещение кормового ПУ",
    "Машинное отделение", "Электростанция и ГРЩ",
    "Служебный блок и цистерны", "Каюты, отсек 1",
    "Каюты, отсек 2 и носовое ПУ", "Форпик, балластные цистерны",
    "Таранный отсек",
]
...
PROP_POWER = G.PROP_MOTOR_COUNT * G.PROP_MOTOR_POWER   # 2 x 1800 = 3600 кВт
