...
# Покупные изделия в PARTS/BOUGHT свалены в одну кучу; спецификация требует
# разнести их по разделам и назвать по обозначению стандарта.

DOCS = [
    dict(fmt="A1", mark=MARK + " СБ", name="Сборочный чертёж"),
    dict(fmt="A2", mark=MARK + " ВО", name="Чертёж общего вида с нагрузками"),
    dict(fmt="A4", mark=MARK + " РР", name="Расчёт фундамента и амортизации"),
]

# формат чертежа детали; БЧ — деталь изготавливается без отдельного чертежа
PART_FORMATS = {1: "A2", 2: "A3", 3: "A3", 4: "БЧ", 5: "A4", 6: "БЧ",
                7: "A4", 8: "БЧ", 9: "A4", 10: "A4"}

STANDARD = [
    ("Болт М20-6gx80.88 ГОСТ 7798-70", 48, ""),
    ("Гайка М20-6Н.8 ГОСТ 5915-70", 48, ""),
    ("Шайба 20.01.08кп ГОСТ 11371-78", 48, ""),
    ("Шайба 20 65Г ГОСТ 6402-70", 48, "пружинная"),
]

OTHER = [
    ("Амортизатор %s, %d кгс" % (ISOLATOR["type"], ISOLATOR["rated_kgf"]),
     N_ISOLATORS, "%d кН/м" % ISOLATOR["stiffness_kN_m"]),
    ("Клапан самозапорный DN50", 1, "слив поддона"),
    ("Перемычка заземляющая 16 мм²", 4, ""),
]


def consumables():
    """Расход сварочных материалов и покрытия на узел."""
    kf = 0.006                      # катет шва, м
    # швы: пояс к стенке по обеим сторонам каждой балки плюс узлы пересечений
    l_long = 2 * N_LONG * (X1 - X0)
    l_cross = 2 * N_CROSS * (Y1 - Y0)
    l_nodes = N_LONG * N_CROSS * 4 * 0.14
    l_tray = 2 * ((X1 - X0) + (Y1 - Y0))
    l_all = l_long + l_cross + l_nodes + l_tray
    a_weld = 0.5 * kf * kf          # площадь сечения углового шва, м²
    wire = a_weld * l_all * 7850.0 * 1.12           # с потерями на угар
    gas = l_all * 0.9               # м³ смеси Ar + CO2 при 12 л/мин
    # окраска: развёрнутая поверхность металла узла
    # развёрнутая поверхность: масса делится на среднюю толщину 8 мм и
    # плотность, обе стороны листа плюс кромки
    area = 2.4 * steel_mass() / (7850.0 * 0.008)
    return dict(weld_m=round(l_all, 1), wire_kg=round(wire, 1),
                gas_m3=round(gas, 0), area_m2=round(area, 1),
                primer_kg=round(area * 0.18, 1), enamel_kg=round(area * 0.22, 1))
...
def specification():
    ...
    c = consumables()
    det = []
    for num, mark, name, n, mat, m in PARTS:
        # в графу «Примечание» идёт только толщина проката: марка стали
        # стоит в основной надписи чертежа детали
        short = mat.split(",")[0].replace("Лист ", "лист ").replace(
            "Полоса ", "полоса ").replace("рифлёный", "рифл.")
        for g in (" ГОСТ 19903", " ГОСТ 103", " ГОСТ 8568"):
            short = short.replace(g, "")
        det.append(dict(fmt=PART_FORMATS.get(num, "A4"), pos=str(num),
                        mark=mark, name=name, qty=str(n), note=short))
    std = [dict(pos="", mark="", name=nm, qty=str(n), note=note)
           for nm, n, note in STANDARD]
    oth = [dict(pos=str(11 + i), mark="", name=nm, qty=str(n),
                note=note[:16])
           for i, (nm, n, note) in enumerate(OTHER)]
    mat = [
        dict(name="Проволока Св-08Г2С-О ⌀1,2 ГОСТ 2246-70",
             qty="%.1f кг" % c["wire_kg"], note="швы %.0f м" % c["weld_m"]),
        dict(name="Смесь защитная Ar + CO₂ ГОСТ Р ИСО 14175",
             qty="%.0f м³" % c["gas_m3"], note="12 л/мин"),
        dict(name="Грунтовка ЭП-0263 ТУ 2312-004", 
             qty="%.1f кг" % c["primer_kg"], note="2 слоя"),
        dict(name="Эмаль ЭП-1155 ТУ 2312-007",
             qty="%.1f кг" % c["enamel_kg"], note="2 слоя, %.0f м²" % c["area_m2"]),
    ]
    return [("Документация",
             [dict(fmt=d["fmt"], mark=d["mark"], name=d["name"]) for d in DOCS]),
            ("Детали", det),
