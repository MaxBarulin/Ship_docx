...
> Генерируется `scripts/записка_фундамента.py` из `src/lib/gorizont_twistlock.py`. Проверок узла 44, не прошло 0. Закрывает блоки 3.1–3.4 и 4.1–4.6 задания.

## 1. Почему этот узел и как он устроен

Команда 22.09.2026 взяла узлом КД фундамент для крепления модульных номеров солнечной палубы. Основа — 3D Глеба (`docs/команда/солнечная_палуба/Чертёж крепления.jpg`): литой корпус на болтах и поворотный конус с рукояткой. Узел держит «трансформер» — 8 модулей на 18 слотах, 72 фундаментов на судно (`gorizont_modules`).
...
- **Высота опоры 135 мм** от настила до низа фитинга. Прежние 0,30 м в `gorizont.MODULES` были заглушкой; теперь модуль с фундаментом 3,04 м при свободных 5,50 м под мостом.
...
Чертёж и 3D собраны из одних чисел (`gorizont_twistlock.КОРПУС`, `ЗАМОК`, `РЫЧАГ`). Масса корпуса по модели сходится с библиотекой до 1,1 %%; сечения на чертежах строятся теми же булевыми операциями, что и тела. Растры листов — `renders/горизонт_2026/чертежи_dxf/ВГ-2026_46_*.png`.
...
Нагрузки на фундамент дают инерция модуля при качке и ветер. Числа качки — из `gorizont_hydro` и `gorizont_roll` (`docs/проект/теория_корабля.md`), перегрузки старого фундамента очистки не используются. Судно очень остойчивое и жёсткое, поэтому период короткий, а ускорения на солнечной палубе высокие. Период зависит от осадки, узел считается по самому короткому:
...
|---|---|---|
| Класс РРР, волна | «О», 2,0 м длиной 20 м | gorizont_hydro.CLASS |
| Амплитуда бортовой на резонансе | 19,8° без успокоителей, 10,9° с цистернами | gorizont_roll.damping() |
| Расчётный период бортовой | 3,26 с — самый короткий в диапазоне осадок 1,20…1,30 м | gorizont_hydro.roll_period |
| Килевая качка | ψ = h/L = 0,82°, период 2,69 с | судно длиной 140 м на волне 2 м |
| Ось качки | через ЦТ судна, z = 2,68 м | gorizont_hydro.equilibrium |
| Ветер | 0,294 кПа на борт модуля | gorizont_hydro.WIND_PRESSURE |
...
![Отливка с прибылью и стержень с ногой-знаком — 3D из CAD/src/twistlock.py](../../renders/горизонт_2026/узел/46_03_отливка_стержень.png)

*Отливка с прибылью и стержень с ногой-знаком — 3D из CAD/src/twistlock.py*
...
Партия занимает меньше трети свободного фонда любого цеха: своих мощностей хватает, новое оборудование — только стенд 250 кН. Кооперация — поковки 40Х и термодиффузионное цинкование. **Принято:** состав оборудования литейки и текущая загрузка цехов; их надо сверить с паспортами цехов АО «Адмиралтейские верфи».
...
|---|---|---|---|---|---|
| РКД узла: чертежи, спецификация, расчёт — согласование с РРР | 08.01.2027 | 04.02.2027 | 20 | ПБ, Глеб | в составе РКД |
| ТП литья, мехобработки и сборки; управляющие программы ЧПУ | 05.02.2027 | 25.02.2027 | 15 | ТУ, ЛЦ, МЦ |  |
| Модельный комплект и стержневой ящик | 03.08.2027 | 30.08.2027 | 20 | модельный участок ЛЦ | задача Лены 7.4 |
| Пробная плавка, 6 отливок установочной партии, РК | 31.08.2027 | 09.09.2027 | 8 | ЛЦ, ОТК |  |
| Мехобработка и сборка установочной партии | 10.09.2027 | 17.09.2027 | 6 | МЦ, СЦ |  |
| Испытания: пробная и разрушающая нагрузка, акт РРР | 20.09.2027 | 27.09.2027 | 6 | ОТК, РРР | освоение |
| Серийная партия: 71 отливок | 01.10.2027 | 28.10.2027 | 20 | ЛЦ | задача Лены 10.4 |
| Мехобработка, покрытие ТДЦ, сборка, испытания серии | 29.10.2027 | 02.12.2027 | 25 | МЦ, К, СЦ |  |
| Подкладные листы на настиле солнечной палубы | 14.06.2028 | 11.07.2028 | 20 | корпусный цех | стапель |
| Установка 72 фундаментов на судне | 02.03.2029 | 15.03.2029 | 10 | достроечный цех | достройка |
...
![Секция люкса А-3-ПБ на крюке над своим слотом: замки открыты, стропальщики держат оттяжки; слева — мелкий бассейн, над которым модуль пронесли](../../renders/горизонт_2026/виды/10_смена_модуля.jpg)
...
![Порт приписки: кран над солнечной палубой, дорожка от вокзала к трапу, фургон съезжает по аппарели на пандус, площадка модулей других тем за тыловым рельсом](../../renders/горизонт_2026/виды/09_у_причала.jpg)
...
- Остальные фитинги из «пачки» Глеба в репозитории не лежат — узел сделан по `Чертёж крепления.jpg` и заданию.
- Кран порта — класса 16 т / 32 м, отметка кордона 1,80 м над водой — приняты для Самары; сверить с паспортом причала.

## 13. Проверки `gorizont_twistlock.checks()`
...
| Высота опоры = высота фундамента в gorizont | 0.135 | 0.135 | ок | корпус 124 + прокладка 3 + лист 8 мм |
...
| Фундаменты готовы до достройки | 02.12.2027 | 05.02.2029 | ок | серия собрана и испытана до начала достройки у Лены |
