...
Нагрузки на фундамент дают инерция модуля при качке и ветер. Числа качки — из `gorizont_hydro` и `gorizont_roll` (`docs/проект/теория_корабля.md`). Судно очень остойчивое и жёсткое, поэтому период короткий, а ускорения на солнечной палубе высокие. Период зависит от осадки, узел считается по самому короткому:
